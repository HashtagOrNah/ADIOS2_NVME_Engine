...
CMAKE="/usr/bin/cmake"
SOURCE_DIR="/home/quinlan/snap/snapd-desktop-integration/current/lab_work/ADIOS2/cmake/install/post/adios2-config-dummy"
BUILD_DIR=$(mktemp -d)
pushd ${BUILD_DIR}

if [ 0 -eq 1 ]
then
  export CC="MPI_C_COMPILER-NOTFOUND"
fi
if [ 0 -eq 1 ]
then
  export CXX="MPI_CXX_COMPILER-NOTFOUND"
fi
if [ 0 -eq 1 ]
then
  export FC="MPI_Fortran_COMPILER-NOTFOUND"
fi

export adios2_DIR="${PREFIX}/lib/cmake/adios2"
if [ -n "" ]
then
...
echo "ADIOS2_CONFIG_MPI=0" >> adios2.flags
...
cat \
  "/home/quinlan/snap/snapd-desktop-integration/current/lab_work/Build/cmake/install/post/adios2-config.pre.sh" \
  adios2.flags \
  "/home/quinlan/snap/snapd-desktop-integration/current/lab_work/Build/cmake/install/post/adios2-config.post.sh" > "${BINDIR}/adios2-config"
